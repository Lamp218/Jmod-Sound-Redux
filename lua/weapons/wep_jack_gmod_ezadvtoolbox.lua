-- Jackarunda 2021
AddCSLuaFile()
SWEP.PrintName = "EZ Advanced Toolbox"
SWEP.Author = "Jackarunda"
SWEP.Purpose = ""
JMod.SetWepSelectIcon(SWEP, "entities/ent_jack_gmod_ezadvtoolbox")
...
function SWEP:Initialize()
	...
	if SERVER then
		...
		for name, info in pairs(JMod.Config.Craftables) do
			if (istable(info.craftingType) and table.HasValue(info.craftingType,"advtoolbox")) or (info.craftingType=="advtoolbox")then
...
function SWEP:OnDrop()
	local Kit = ents.Create("ent_jack_gmod_ezadvtoolbox")
